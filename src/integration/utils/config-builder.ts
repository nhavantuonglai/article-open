...
const defaultConfig = {
  ...
  image: {
    imageSizes: [16, 32, 48, 64, 96, 128, 256, 384], // Xóa 'Lisa', đảm bảo tất cả là số
...
// Xuất các config riêng lẻ
export const SITE = getConfig('site', defaultConfig.site);
export const I18N = getConfig('i18n', defaultConfig.i18n);
export const METADATA = getConfig('metadata', defaultConfig.metadata);
export const APP_BLOG = getConfig('apps.blog', defaultConfig.apps.blog);
export const UI = getConfig('ui', defaultConfig.ui);
export const ANALYTICS = getConfig('analytics', defaultConfig.analytics);
export const IMAGE_CONFIG = getConfig('image', defaultConfig.image);
